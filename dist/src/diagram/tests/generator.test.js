...
describe("ignore functionality", () => {
    it("ignores nodes with cdk-dia:ignore metadata annotation", () => {
        ...
        const diagramJson = diagram.toSimpleObject();
        const flattenedComponents = [];
        const collectAllComponents = (component) => {
        ...
        const allComponentIds = flattenedComponents.map((c) => c.id);
        expect(allComponentIds.some((id) => id.includes("NormalResource"))).toBe(true);
        expect(allComponentIds.some((id) => id.includes("IgnoredResource"))).toBe(false);
    });
});
